...
// Keeps "pay the registration fee" firing while "we never ask for any registration
// fee — beware of scammers" stays silent.
...
function buildContactSignals(claims, output) {
  ...
  if (hasUnnegatedTerm(combined, ['telegram', 't me'])) {
  ...
  if (hasUnnegatedTerm(combined, ['whatsapp', 'wa me'])) {
    addUnique(output, signal(
  ...
  if (hasUnnegatedTerm(combined, ['viber'])) {
    addUnique(output, signal(
      'contact.viber_only',
      'contact',
      'risk',
...
// fee/charge variants scammers rotate through, plus deposit/purchase phrasings.
...
// "No interview" phrased every way scammers phrase it (incl. Taglish).
...
function buildProcessSignals(claims, output) {
  const appPath = normalize(claims?.applicationPath)
  if (NO_VETTING_TERMS.some((term) => hasTokenPhrase(appPath, term)) || appPath.includes('no interview')) {
    addUnique(output, signal(
  ...
  if (hasUnnegatedTerm(paymentContext, UPFRONT_PAYMENT_TERMS)) {
    addUnique(output, signal(
...
export function traceAuditSignals(signals, evidence = [], weightOverrides = undefined) {
  ...
  const hasOffPlatformContactSignal = (
    ids.has('contact.telegram_only') || ids.has('contact.whatsapp_only') || ids.has('contact.viber_only')
  )
