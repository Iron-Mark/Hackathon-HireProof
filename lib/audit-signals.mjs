...
function buildProcessSignals(claims, output) {
  ...
  if (UPFRONT_PAYMENT_TERMS.some((term) => paymentContext.includes(term))) {
    addUnique(output, signal(
...
export function scoreAuditSignals(signals, evidence = [], weightOverrides = undefined) {
  ...
  let score = 25 + safeSignals.reduce((total, item) => total + effectiveSignalWeight(item, weightOverrides), 0)
  ...
  if (ids.has('entity.apply_path_mismatch') || ids.has('domain.apply_mismatch') || ids.has('entity.input_conflict')) score = Math.max(score, 45)
