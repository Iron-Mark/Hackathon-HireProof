...
type ProviderCostName = 'model' | 'serpapi' | 'googleVision' | 'safeBrowsing'

type ProviderCostGuardResult = {
  allowed: boolean
  remaining: number
...
const DEFAULT_DAILY_LIMITS: Record<ProviderCostName, number> = {
  model: 50,
  serpapi: 100,
  googleVision: 50,
  safeBrowsing: 500,
...
const ENV_LIMITS: Record<ProviderCostName, string> = {
...
function dailyLimit(provider: ProviderCostName) {
...
function keyFor(provider: ProviderCostName) {
  return `hireproof:provider-cost:${provider}:${dayKey()}`
}

function message(provider: ProviderCostName, limit: number) {
  return `Daily ${provider} platform provider limit reached (${limit}/day). Use BYOK credentials or try again after the daily reset.`
}

export async function checkProviderCostGuard(provider: ProviderCostName): Promise<ProviderCostGuardResult> {
  ...
  if (redis) {
    try {
      ...
      // Fallback to local counters so cost protection still exists during Redis issues.
    }
  }
...
export function getProviderCostGuardSnapshot() {
  return {
    ...
    flags: {
      ...
      publicTrendsExternalSignalsEnabled: process.env.PUBLIC_TRENDS_EXTERNAL_SIGNALS_ENABLED === 'true',
