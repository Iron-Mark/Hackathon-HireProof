...
// Company mentioned after "at/from/with": a bounded proper-noun sequence — up to 4 capitalized words,
// with the connectors of/the/and/for/& allowed between them (so "Bank of America" works). The capture
// self-limits at the first non-capitalized word, so a run-on clause like "Google Play gift cards and
// message us on WhatsApp" can never be swallowed whole as a company name.
const COMPANY_AFTER_PREPOSITION_RE = /(?:at|from|with)\s+([A-Z][A-Za-z0-9&'-]*(?:\s+(?:(?:of|the|and|for|&)\s+)?[A-Z][A-Za-z0-9&'-]*){0,3})/
...
// Recognises a genuine official apply channel by PHRASING only: a careers PAGE / PORTAL / SITE /
// SECTION / HUB (singular or plural), or an "official <careers|website|company|portal|page|hiring|
// application|apply>" phrase. Deliberately NOT URL/email parsing — real apply URLs arrive in the
// `url` field and short-circuit to "Provided job URL" above, so matching careers-shaped tokens in
// prose only opened an injection surface: a "careers@"/"careers.hr.team@gmail.com" email local
// part, a "bit.ly/careers" shortener, an "/careers-fair" event slug, or a "careers.2024.pdf"
// filename would each forge an unearned "recognizable official channel" trust signal and discount
// the score. The word separator is a literal space or non-breaking space ([ \u00a0]+, not \s+): a
// newline or tab would otherwise glue an unrelated heading/column ("careers" \n "Page 1 of 3", or
// "status: official" \t "Careers info") into a false match. Plural "careers" after "official"
// avoids the "official career offer" lure; and "channel" is excluded (a Telegram/WhatsApp "careers
// channel" is not a careers page). A bare "official" alone never matches — it needs a real noun.
const OFFICIAL_CAREERS_CHANNEL_RE =
  /\bcareers?[ \u00a0]+(?:page|portal|site|website|section|hub|centre|center)s?\b|\bofficial[ \u00a0]+(?:careers|web[ \u00a0]?sites?|company(?:[ \u00a0]+website)?|portals?|pages?|hiring|application|apply)\b/

export function isOfficialCareersChannel(text) {
  return OFFICIAL_CAREERS_CHANNEL_RE.test(String(text || '').toLowerCase())
}
...
function extractApplicationPath(text, url) {
  ...
  if (hostMatchesDomain(host, 'linkedin.com')) return lower.includes('easy apply') ? 'LinkedIn Easy Apply' : 'LinkedIn job page'
  if (hostMatchesDomain(host, 'greenhouse.io')) return 'Greenhouse job page'
  if (hostMatchesDomain(host, 'lever.co')) return 'Lever job page'
  if (hostMatchesDomain(host, 'ashbyhq.com')) return 'Ashby job page'
  if (hostMatchesDomain(host, 'smartrecruiters.com')) return 'SmartRecruiters job page'
  if (hostMatchesDomain(host, 'workdayjobs.com') || hostMatchesDomain(host, 'myworkdayjobs.com')) return 'Workday job page'
  ...
  if (isOfficialCareersChannel(text)) return 'Official careers channel'
  return null
}
...
function extractFirstMatch(text, patterns, fallback = 'Unknown') {
  for (const pattern of patterns) {
...
/**
 * Deterministic, model-free extraction of structured claims from a raw job post.
 * This is the single source of truth for the audit route's `useModel:false` path and the
 * real-text eval harness — keep them one and the same so extraction can be measured end-to-end.
 * @param {{ text?: string, url?: string, location?: string }} input
 */
