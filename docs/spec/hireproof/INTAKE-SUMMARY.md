...
Verified on `2026-06-14` from the archived pro-research source pack, current repo source, and targeted repo tests.
...
| Identify and optimize the real LCP element | implemented | `scripts/check-web-vitals.mjs` now records the LCP element per route; latest local production report identifies text LCP on `/`, `/audit`, `/docs`, and `/lab`, and `H1` on `/demo/linkedin`. |
...
| Add Lighthouse/PageSpeed/field Web Vitals measurement gate | implemented | Added `scripts/check-web-vitals.mjs` and package script `proof:web-vitals`; latest local production report passed all measured routes. |
