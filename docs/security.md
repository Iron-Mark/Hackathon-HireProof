...
- **Auth Throttling:** Login and registration use per-email buckets plus client-identity buckets. Forwarding headers such as `X-Real-IP` and `X-Forwarded-For` are ignored unless `TRUST_PROXY_CLIENT_IP_HEADERS=true` is set behind a proxy that strips or overwrites client-supplied values; malformed forwarding values are rejected unless they parse as real IP literals.
