...
test('multi-platform chat agents are wired through ChatSDK adapters', async () => {
  ...
  assert.match(chatRoute, /validateMutationOrigin/)
  assert.match(chatRoute, /CHAT_PAYLOAD_LIMIT_BYTES/)
  assert.match(chatRoute, /checkRateLimit\(`chat_hireproof:\$\{requestIp\(request\)\}`/)
  assert.match(chatRoute, /AuditRequestSchema\.safeParse/)
  assert.match(chatRoute, /persist: false/)
...
test('workflow route uses the WDK package and next plugin is enabled', async () => {
  ...
  const workflowSecret = await fs.readFile(new URL('../lib/workflow-secret.ts', import.meta.url), 'utf8')
  ...
  const credentialsDocs = await fs.readFile(new URL('../docs/credentials-setup.md', import.meta.url), 'utf8')
  ...
  assert.match(envExample, /WORKFLOW_SECRET=\s*(?:\r?\n|$)/)
  assert.match(deployment, /WORKFLOW_SECRET=\s*(?:\r?\n|$)/)
  assert.match(credentialsDocs, /WORKFLOW_SECRET=\s*(?:\r?\n|$)/)
