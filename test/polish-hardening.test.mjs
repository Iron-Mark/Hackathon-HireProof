...
test('demo login response only returns public demo identity', async () => {
  ...
  assert.match(route, /validateMutationOrigin\(request\)/)
  assert.match(route, /requestIp\(request\)/)
  assert.doesNotMatch(route, /new URL\(origin\)/)
  assert.doesNotMatch(route, /new URL\(referer\)/)
  assert.match(route, /CSRF validation failed|validateMutationOrigin/)
  ...
  assert.doesNotMatch(route, /reqHeaders\.get\('x-real-ip'\)/)
  assert.doesNotMatch(route, /reqHeaders\.get\('x-forwarded-for'\)/)
